...
from screens import admin, user  # noqa: E402
...
class Stand:
    ...
    async def boot(self, renderer: Renderer, world: FakeWorld, key: str, db_path: Path) -> "Stand":
        ...
        for r in (access.claim_router, admin.router, user.router):  # «перезапуск» в том же процессе
...
async def scenarios(st: Stand) -> None:
    ...
    check("Мама" in st.caption(BOB) and "[🖼 фото] смотри" in st.caption(BOB), "переписка открыта, вложения подписаны")
...
async def random_walk(st: Stand, n: int) -> None:
    ...
    for i in range(n):
        ...
        if (i % 200 == 100 or i == n - 1) and st.hub.accs.get(1) and st.hub.accs[1].status == "need_login":
            await st.press(ADMIN, "relog:1")
            await keypad(st, ADMIN, "12345")
    a = st.hub.accs.get(1)  # перед проверкой перезапуска аккаунт должен быть рабочим
    if a and not a.enabled:
        await st.press(ADMIN, "aat:1")
    if a and a.status == "need_login":
        await st.press(ADMIN, "relog:1")
        await keypad(st, ADMIN, "12345")
